...
def compute_support_resistance(df: pd.DataFrame):
    ...
    swing_high = safe_number(hh20_s.iloc[-1] if pd.notna(hh20_s.iloc[-1]) else high_s.tail(20).max(), default=None)
    swing_low  = safe_number(ll20_s.iloc[-1] if pd.notna(ll20_s.iloc[-1]) else low_s.tail(20).min(),  default=None)
...
def determine_signal(latest: pd.Series, support: float, resistance: float, volume_threshold: float):
    """
    ...
    volume_spike    = volume is not None and volume_threshold is not None and volume > volume_threshold
    ...
    vwap_ok_long    = (vwap_diff_bps is None) or (-VWAP_NEAR_BPS <= vwap_diff_bps <= VWAP_NEAR_BPS or vwap_diff_bps > 0)
    vwap_ok_short   = (vwap_diff_bps is None) or (-VWAP_NEAR_BPS <= vwap_diff_bps <= VWAP_NEAR_BPS or vwap_diff_bps < 0)
    ...
    if near_resistance and volume_spike:
        reasons.append(f"Price near resistance {round(resistance,2)} with volume spike.")
        if adx_good: reasons.append(f"ADX {round(adx14,1)} strong.")
        if slopes_bull: reasons.append("EMA20/50 slopes positive.")
        if macd_up: reasons.append("MACD histogram rising.")
        if bb_squeeze: reasons.append("BB squeeze → potential expansion.")
        if bb_conf_long: reasons.append("Price above upper BB.")
        if atr_ok: reasons.append(f"ATR {round(atr_pct,2)}% supports move.")
        if vwap_ok_long: reasons.append("VWAP alignment OK.")

        strong_checks = sum([
            adx_good, slopes_bull, macd_up, bb_conf_long, atr_ok
        ])
        if strong_checks >= 3:
    ...
    # --- Short / breakdown bias ---
        ...
        if adx_good: reasons.append(f"ADX {round(adx14,1)} strong.")
        if slopes_bear: reasons.append("EMA20/50 slopes negative.")
        if macd_down: reasons.append("MACD histogram falling.")
        if bb_squeeze: reasons.append("BB squeeze → potential expansion.")
        if bb_conf_short: reasons.append("Price below lower BB.")
        if atr_ok: reasons.append(f"ATR {round(atr_pct,2)}% supports move.")
        if vwap_ok_short: reasons.append("VWAP alignment OK.")

        strong_checks = sum([
            adx_good, slopes_bear, macd_down, bb_conf_short, atr_ok
        ])
        if strong_checks >= 3:
...
def basic_forecast_update(stock_cfg, recent_csv, historical_folder="historical_data"):
    """
    Fallback forecast using recent + historical data (last ~10 daily bars if present).
    Uses new indicator columns when available.
    Returns JSON in the SAME schema your LLM uses (no new keys added).
    """
    stock_cfg = copy.deepcopy(stock_cfg)
    stock_code = stock_cfg.get("stock_code")

    # --- Load historical (auto-find, tail 10) ---
    ...
    if hist_csv and os.path.exists(hist_csv):
        try:
            df_hist = pd.read_csv(hist_csv).tail(10)
        except Exception:
            df_hist = pd.DataFrame()

    # --- Load recent (live) ---
    df_recent = pd.DataFrame()
    if recent_csv and os.path.exists(recent_csv):
        try:
    ...
    if df_recent.empty:
        # nothing to do; return untouched config
        stock_cfg["last_updated"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        return stock_cfg

    # --- Merge for computing SR ---
    df_all = pd.concat([df_hist, df_recent], ignore_index=True) if not df_hist.empty else df_recent.copy()

    # Ensure we have reasonable column case coverage
    # (Your real-time file likely has TitleCase; historical has lowercase)
    # compute S/R
    support, resistance, bb_upper, bb_lower = compute_support_resistance(df_all)

    # Volume threshold: robust to different cases
    vol_series = _get(df_recent, "Volume", "volume", default=0)
    volume_threshold = float(vol_series.tail(50).mean() * 1.5) if len(vol_series) else 0.0

    # Latest row for decision (last of the recent file)
    latest = df_recent.iloc[-1]
    signal, reasons = determine_signal(latest, support, resistance, volume_threshold)

    # --- Return SAME schema you already use ---
    return {
        "stock_code": stock_cfg.get("stock_code"),
